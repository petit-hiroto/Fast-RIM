...
def create_extraction_prompt(text):
    # この関数は、面接の内容から情報を抽出するための指示を作ります。
    return f"""
    この文章はとある面接の内容です。
    以下の項目を抽出してください：
...
def load_prompt_from_settings(log=True):
...
def transcribe_audio_with_key(audio_file, api_key, retries=3):
    ...
    logging.debug(f"文字起こしを開始します: {audio_file} with API key: {api_key}")
...
def create_excel(extracted_info, output_file):
    ...
    # 列の幅を設定します
    ws.column_dimensions['A'].width = 20  # A列（項目名）の幅を20に設定
    ...
    # 面接詳細情報をExcelに書き込みます
    for i, detail in enumerate(interview_details, start=1):
        ...
        ws.cell(row=i, column=1).alignment = Alignment(vertical='top')  # 上寄せに設定
    ...
    row = len(interview_details) + 1  # 面接詳細情報の後から項目の書き込みを開始します
    ...
    for row in ws['A1:B'+str(ws.max_row)]:
        for cell in row:
            ...
            if cell.column == 1:  # A列（項目名）のセルの場合
                ...
                cell.alignment = Alignment(vertical='top')  # 上寄せに設定
            elif cell.column == 2:  # B列（内容）のセルの場合
                cell.alignment = Alignment(wrap_text=True, vertical='top')  # テキストを折り返して表示し、上寄せに設定
...
def process_audio_file(audio_file_path, processed_files):
    try:
        ...
        logging.debug(f"音声ファイルを{num_parts}個に分割します。")
        audio_parts = split_audio_file(audio_file_path, num_parts)
        logging.debug(f"分割されたファイル: {audio_parts}")
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_index):
                ...
                try:
                    ...
                    logging.error(f"{part}の処理中にエラーが発生しました: {str(e)}")
                    failed_parts.append((index, part))
...
def extract_info_from_xlsx(file_path):
    ...
    sheet = wb.active
    data = {
        '日時': format_date(sheet['B1'].value),
        '場所': sheet['B2'].value or '',
        '候補者': sheet['B3'].value or '',
        '面接担当者': sheet['B4'].value or '',
        '候補者の強み': sheet['B5'].value or '',
...
def show_main_menu(reload_prompt=False):
    ...
    if reload_prompt:
        transcription_prompt = load_prompt_from_settings(log=True)  # ログ出力あり
    ...
    # タイトルラベル
    title_label = tk.Label(root, text="Fast RIM", font=("Arial Black", 32, "bold"))
...
def process_xlsx_file_async(xlsx_file):
    template_path = os.path.join(get_current_dir(), '面接議事録テンプレート.docx')  # dist直下から取得
    output_directory = load_output_directory()
...
def main():
    ...
    try:
        ...
        show_main_menu(reload_prompt=False)  # 初回起動時は再読み込みしない
...
def get_settings_path():
    ...
    return Path.home() / ".my_RIM_app" / "settings.json"
...
def ensure_settings_exist():
    settings_path = Path.home() / ".my_RIM_app" / "settings.json"
